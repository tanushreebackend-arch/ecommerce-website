    ...
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-md bg-white z-50 shadow-2xl flex flex-col slide-up">
        <div className="flex items-center justify-between p-4 border-b">
          <h2 className="font-heading text-lg font-normal">Cart • {items.length} items</h2>
          <button onClick={closeCart} className="p-2 hover:bg-[#F5F5F5] rounded-lg">
            <X size={20} />
          </button>
        </div>
        ...
        {items.length === 0 ? (
          ...
          <>
            ...
            <div className="px-4 py-4 bg-[#F5F5F5] border-b border-[#E8E8E8]">
              {freeShipping ? (
                <p className="text-sm font-normal text-[#0A0A0A] mb-3">
                  Congrats! You get <span className="font-medium">FREE shipping!</span>
                </p>
              ) : (
                <p className="text-sm font-normal text-[#0A0A0A] mb-3">
              ...
              <div className="relative h-7">
                ...
                <div
                  ...
                  aria-hidden
                >
                  <Truck size={13} strokeWidth={2.25} className="text-[var(--color-heading)]" />
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {items.map((item) => (
                <div key={item.id} className="flex gap-3 pb-4 border-b">
                  <div className="relative w-20 h-20 rounded-lg overflow-hidden shrink-0 bg-[#F5F5F5]">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs text-[#999999] line-through">₹{item.originalPrice.toLocaleString('en-IN')}</span>
                    </div>
                    <p className="text-xs text-[var(--color-text-secondary)]">(You save ₹{((item.originalPrice - item.price) * item.quantity).toLocaleString('en-IN')})</p>
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex items-center border rounded">
                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1 hover:bg-[#F5F5F5]">
                          <Minus size={14} />
                        </button>
                      ...
                      <button onClick={() => removeItem(item.id)} className="p-1 text-[#555555] hover:bg-[#F5F5F5] rounded">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
            ...
            <div className="p-4 border-t space-y-3">
              {savings > 0 && (
                <div className="flex justify-between text-sm font-normal text-[var(--color-heading)]">
